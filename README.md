...
I provided a [`build.ninja`](https://ninja-build.org/manual.html) file that uses clang 18 as the C and C++ compiler. You can change the compiler in the build file, but this program is not exactly the most portable program: if you switch the compiler, be aware that the program uses Intel compiler style intrinsics (`__mm256_frobnicate_epu32`). Also, this program depends on `pthread` and `posix_memalign`. It's not designed with non-Unix systems in mind. Running `ninja` in the root directory for the project should create the library `bin/mediocre.so` for the project.
